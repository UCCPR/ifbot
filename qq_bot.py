...
def send_qq_message(channel_id: str, content: str = "", image_url: str = None, msg_id: str = None) -> bool:
    """
    通过 QQ 开放平台 API 发送消息到频道
    :param channel_id: 频道ID
    :param content: 文本内容
    :param image_url: 图片URL（可选，需先通过 upload_qq_media 上传获取）
    :param msg_id: 要回复的消息ID（可选）
    """
    if not channel_id:
        log_error("发送消息失败：缺少channel_id")
        return False

    # 检查速率限制
    if not check_message_rate(channel_id):
        log_error(f"消息发送被速率限制阻止，channel: {channel_id}")
        return False
    ...
    try:
        url = f"{QQ_API_BASE}/v2/channels/{channel_id}/messages"
        headers = {
            "Authorization": f"QQBot {QQ_BOT_TOKEN}",
        ...
        if response.status_code == 200:
            ...
            if result.get("code") == 0:
                ...
                log_error(f"QQ API返回错误: {result.get('message', 'unknown')}")
                return False
        else:
            log_error(f"发送失败 HTTP{response.status_code}: {response.text[:200]}")
            return False

    except requests.exceptions.ConnectionError:
        log_error(f"连接QQ API失败，请检查网络")
        return False
    except Exception as e:
...
def upload_qq_media(channel_id: str, file_bytes: BytesIO, filename: str, content_type: str) -> str:
    """
    上传媒体文件到QQ频道，返回URL
    :param channel_id: 频道ID（用于上传目标）
    :param file_bytes: 文件BytesIO对象
    :param filename: 文件名
    :param content_type: MIME类型
    :return: 文件URL，失败返回None
    """
    try:
        url = f"{QQ_API_BASE}/v2/channels/{channel_id}/files"
        headers = {"Authorization": f"QQBot {QQ_BOT_TOKEN}"}
        ...
        if response.status_code == 200:
            ...
            if result.get("code") == 0:
                ...
                log_error(f"上传文件失败: {result.get('message', 'unknown')}")
                return None
        else:
            log_error(f"上传文件失败 HTTP{response.status_code}: {response.text[:200]}")
            return None
    except Exception as e:
        log_error(f"上传文件异常: {e}")
        return None
...
def send_qq_image(channel_id: str, image: Image.Image, content: str = "") -> bool:
    ...
    :param content: 附带文本（可选）
    """
    buffer = BytesIO()
    image.save(buffer, format="PNG")
    buffer.seek(0)
    image_url = upload_qq_media(channel_id, buffer, "image.png", "image/png")
    if not image_url:
        return False
    return send_qq_message(channel_id, content=content, image_url=image_url)


def send_qq_gif(channel_id: str, gif_buffer: BytesIO, content: str = "") -> bool:
    """
    上传GIF BytesIO并发送到QQ频道
    :param channel_id: 频道ID
    :param gif_buffer: GIF BytesIO对象
    :param content: 附带文本（可选）
    """
    image_url = upload_qq_media(channel_id, gif_buffer, "battle.gif", "image/gif")
    if not image_url:
        return False
    return send_qq_message(channel_id, content=content, image_url=image_url)
...
def send_message(message: str, user_id: str = None, group_id: str = None, image_path: str = None):
    ...
    统一消息发送入口（兼容旧接口）
    :param message: 消息内容
    :param user_id: 用户/频道ID
    :param group_id: 群/频道ID（优先使用）
    :param image_path: 图片路径（已废弃，改用 send_qq_image）
    """
    channel_id = group_id if group_id else user_id
    if not channel_id:
        log_error("发送消息失败：缺少channel_id")
        return False
    ...
    if image_path:
        try:
            ...
            image_url = upload_qq_media(channel_id, img_bytes, os.path.basename(image_path), "image/png")
            if image_url:
                return send_qq_message(channel_id, content=message, image_url=image_url)
        except Exception as e:
            log_error(f"从路径发送图片失败: {e}")

    return send_qq_message(channel_id, content=message)


def send_image_from_path(channel_id: str, img_path: str, content: str = "") -> bool:
    """从文件路径读取图片并通过QQ API发送（替代旧的CQ码方式）"""
    try:
        img = Image.open(img_path)
        result = send_qq_image(channel_id, img, content=content)
        try:
        ...
        log_error(f"从路径发送图片失败: {e}")
        return send_qq_message(channel_id, content=content)


def send_message_with_image(channel_id: str, text: str, img_path: str) -> bool:
    """
    发送文字+图片（替代旧的 CQ image 嵌入模式）
    先发文字，再发图片
    """
    ok1 = send_qq_message(channel_id, content=text) if text.strip() else True
    ok2 = send_image_from_path(channel_id, img_path)
    return ok1 and ok2


def send_image(image_path: str, user_id: str = None, group_id: str = None):
    """发送图片消息（通过QQ API上传，发送后自动清理）"""
    try:
        if not os.path.exists(image_path):
            log_error(f"图片文件不存在: {image_path}")
            return False
        channel_id = group_id if group_id else user_id
        return send_image_from_path(channel_id, image_path)
    except Exception as e:
...
def handle_message():
    """
    处理 QQ 开放平台 Webhook 消息
    GET:  健康检查
    POST: 事件处理 / 签名验证
    """
    if request.method == 'GET':
        return jsonify({"status": "ok", "bot": "qq_bot", "time": str(datetime.now())})
    ...
    try:
        ...
        log_info(f"Webhook: op={op} type={req_type}")
        ...
        if op == 13:
            ...
            log_info(f"URL验证(op=13): token={plain_token[:8]}... ts={event_ts}")

            from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
            secret = QQ_BOT_SECRET if QQ_BOT_SECRET else ""
            seed = secret.encode("utf-8")[:32].ljust(32, b'\x00')
            msg = (event_ts + plain_token).encode("utf-8")
            sk = Ed25519PrivateKey.from_private_bytes(seed)
            signature = sk.sign(msg).hex()
            log_info(f"Ed25519签名: {signature[:32]}...")
            return jsonify({"plain_token": plain_token, "signature": signature})

        # ===== URL地址验证: type=sign 格式 =====
        if req_type == "sign":
            echo = data.get("echo", "")
            d = data.get("data", {})
            event_ts = d.get("eventTs", "")
            plain_token = d.get("plainToken", "")
            log_info(f"URL验证(type=sign): ts={event_ts} token={plain_token[:8]}...")
            ...
            sk = Ed25519PrivateKey.from_private_bytes(seed)
            signature = sk.sign(msg).hex()
            return jsonify({"echo": echo, "data": {"signature": signature}})

        # ===== 运行时消息：不强制校验签名（先跑通功能） =====

        # ===== 解析消息事件 =====
        # QQ Webhook 消息事件格式: type=message, data={...}
        event_type = data.get("type", "")
        event_data = data.get("data", {})

        if event_type not in ("message", "group_message", "direct_message", ""):
            return jsonify({"status": "ignored", "type": event_type})

        raw_message = event_data.get("content", "").strip()
        user_id = event_data.get("author", {}).get("id") or event_data.get("user_id", "unknown")
        ...
        is_at = f"@{QQ_BOT_APP_ID}" in raw_message or event_data.get("is_at", False)
        is_dm = event_type == "direct_message"
        if is_dm:
            is_at = True

        log_info(f"MSG uid={user_id} ch={channel_id}: {raw_message[:60]}")

        if user_id and user_id != 'unknown':
            DAILY_USER_SET.add(str(user_id))
        save_daily_stats()

        self_id = str(QQ_BOT_APP_ID) if QQ_BOT_APP_ID else None
        ...
        if has_box_session(user_id):
            ...
            # 移除艾特CQ码
            cleaned_message = cleaned_message.replace(f'<@{self_id}>', '').strip()
            # 移除@符号（如果有的话）
        ...
        if str(user_id) == str(ADMIN_QQ):
            ...
            at_match_admin = re_admin.search(r'<@(\d+)>', raw_message)
...
"""
        ...
        if img_path and os.path.exists(img_path):
            ...
            if group_id and user_id:
                reply = f"<@{user_id}> {reply}"
            send_qq_message(group_id or user_id, content=reply)
    ...
    try:
        ...
        if gen_gif and BATTLE_SYSTEM_LOADED:
            try:
                ...
                if gif_buffer:
                    send_qq_gif(channel_id, gif_buffer, content="战斗回放")
                else:
                    send_qq_message(channel_id, content="GIF生成失败")
        ...
        if '自动配队' in raw_message or '自动' in raw_message:
            ...
            if img_path and os.path.exists(img_path):
                try:
                    ...
                    at = f"<@{user_id}> " if group_id and user_id else ""
                    send_qq_message(group_id or user_id, content=f"{at}🤖 自动配队完成")
            else:
                at = f"<@{user_id}> " if group_id and user_id else ""
                send_qq_message(group_id or user_id, content=f"{at}🤖 自动配队完成")
        ...
        if '我的卡' in raw_message:
            ...
            if img_path and os.path.exists(img_path):
                ...
                if group_id and user_id:
                    ...
                    at_message = ""
                send_qq_message(group_id or user_id, content=f"{at_message}{page_info}\n{usage_hint}")
                send_image_from_path(group_id or user_id, str(img_path))
            ...
            if m:
                ...
                if load_preset(user_id, slot):
                    send_qq_message(group_id or user_id, content=f"<@{user_id}> 已切换到预设{slot}！" if group_id and user_id else f"已切换到预设{slot}！")
                    team_data = load_team_data(user_id)
                    img_path = build_team_image(team_data, characters)
                    if img_path and os.path.exists(img_path):
                        send_image_from_path(group_id or user_id, str(img_path))
                    ...
                    if result["success"] and result["team"]:
                        ...
                        if img_path and os.path.exists(img_path):
                            if group_id and user_id:
                                send_qq_message(group_id or user_id, content=f"<@{user_id}> 预设{slot}为空，正在自动配队...\n{reply}")
                            send_image_from_path(group_id or user_id, str(img_path))
                            os.remove(img_path)
                        else:
                            if group_id and user_id:
                                reply = f"<@{user_id}> 预设{slot}为空，正在自动配队...\n{reply}"
                            send_qq_message(group_id or user_id, content=reply)
                    else:
            ...
            if img_path and os.path.exists(img_path):
                ...
                hints = ("队伍 预设 | 队伍 切换 1~6 | "
                         "队伍 我的卡 | 队伍 自动配队")
                send_qq_message(group_id or user_id, content=hints)
            else:
                reply = "队伍配置为空！"
                if group_id and user_id:
                    reply = f"<@{user_id}> {reply}"
...
def handle_defense_team(user_id: str, group_id, raw_message: str):
    ...
    try:
        ...
        if img_path and os.path.exists(img_path):
            ...
            send_qq_message(group_id or user_id, content=f"{at_message}{info}")
